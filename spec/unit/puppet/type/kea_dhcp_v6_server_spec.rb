...
describe Puppet::Type.type(:kea_dhcp_v6_server) do
  ...
  it 'only allows the dhcp6 instance name' do
    expect {
      described_class.new(name: 'default', lease_database: base_lease_db)
    }.to raise_error(Puppet::ResourceError, %r{Only the 'dhcp6'})
  end
  ...
  it 'rejects invalid option entries' do
    expect {
      described_class.new(name: 'dhcp6', lease_database: base_lease_db, options: ['invalid'])
    }.to raise_error(Puppet::ResourceError, %r{Each option must be a hash})
  end

  it 'rejects lease databases without required keys' do
    expect {
      described_class.new(name: 'dhcp6', lease_database: { 'type' => 'postgresql' })
  ...
  describe 'options property' do
    it 'rejects options missing the name key' do
      expect {
        described_class.new(name: 'dhcp6', lease_database: base_lease_db, options: [{ 'data' => '2001:db8::1' }])
    ...
    it 'rejects options missing the data key' do
      expect {
        described_class.new(name: 'dhcp6', lease_database: base_lease_db, options: [{ 'name' => 'dns-servers' }])
      }.to raise_error(Puppet::ResourceError, %r{Each option must be a hash containing at least name and data})
    end
    ...
    it 'defaults to an empty array' do
      resource = described_class.new(name: 'dhcp6', lease_database: base_lease_db)
  ...
  describe 'hooks_libraries property' do
    ...
    it 'rejects hooks libraries that are not hashes' do
      expect {
        described_class.new(name: 'dhcp6', lease_database: base_lease_db, hooks_libraries: ['invalid'])
    ...
    it 'rejects hooks libraries missing the library key' do
      expect {
        described_class.new(name: 'dhcp6', lease_database: base_lease_db, hooks_libraries: [{ 'parameters' => {} }])
      }.to raise_error(Puppet::ResourceError, %r{Each hooks library must be a hash containing at least a library key})
    end
  ...
  describe 'lease_database property' do
    it 'rejects non-hash values' do
      expect {
        described_class.new(name: 'dhcp6', lease_database: 'invalid')
    ...
    ['name', 'user', 'password', 'host', 'port'].each do |required_key|
      it "requires the #{required_key} key" do
        ...
        expect {
          described_class.new(name: 'dhcp6', lease_database: incomplete_db)
  ...
  describe 'config_path parameter' do
    it 'defaults to /etc/kea/kea-dhcp6.conf' do
      resource = described_class.new(name: 'dhcp6', lease_database: base_lease_db)

      expect(resource[:config_path]).to eq('/etc/kea/kea-dhcp6.conf')
    end
    ...
    it 'accepts a custom path' do
      resource = described_class.new(
        name: 'dhcp6',
        lease_database: base_lease_db,
        config_path: '/custom/path/kea-dhcp6.conf',
      )

      expect(resource[:config_path]).to eq('/custom/path/kea-dhcp6.conf')
    end
  end
  ...
  describe 'autorequire' do
    it 'autorequires the config file' do
      ...
      config_file = Puppet::Type.type(:file).new(name: '/etc/kea/kea-dhcp6.conf', ensure: :file)
      server = described_class.new(name: 'dhcp6', lease_database: base_lease_db)
