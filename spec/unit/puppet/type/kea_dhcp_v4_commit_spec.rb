...
describe Puppet::Type.type(:kea_dhcp_v4_commit) do
  it 'uses name as the namevar (config path)' do
    r = described_class.new(name: '/etc/kea/kea-dhcp4.conf')
    expect(r[:name]).to eq('/etc/kea/kea-dhcp4.conf')
  end
  ...
  describe 'applied insync?' do
    let(:resource) { described_class.new(name: '/etc/kea/kea-dhcp4.conf') }
