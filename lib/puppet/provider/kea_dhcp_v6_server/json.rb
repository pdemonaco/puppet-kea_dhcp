...
Puppet::Type.type(:kea_dhcp_v6_server).provide(:json, parent: PuppetX::KeaDhcp::Provider::Dhcp6Json) do
  desc 'Manages the server level Kea DHCPv6 configuration stored in kea-dhcp6.json.'
  ...
  def self.dhcp6_config(config)
    config.fetch(self::DHCP6_KEY, {})
  end
  ...
  def flush
    ...
    dhcp6 = config[self.class::DHCP6_KEY]

    if @property_flush[:ensure] == :absent
      dhcp6.delete(self.class::OPTION_DATA_KEY)
      dhcp6.delete(self.class::HOOKS_LIBRARIES_KEY)
      dhcp6.delete(self.class::LEASE_DATABASE_KEY)
      ensure_state = :absent
    else
      dhcp6[self.class::OPTION_DATA_KEY] = Array(value_for(:options)).map { |opt| stringify_keys(opt) }
      hooks_libs = Array(value_for(:hooks_libraries)).map { |hook| deep_stringify_keys(hook) }
      dhcp6[self.class::HOOKS_LIBRARIES_KEY] = hooks_libs unless hooks_libs.empty?
      lease_db = stringify_keys(value_for(:lease_database))
      dhcp6[self.class::LEASE_DATABASE_KEY] = lease_db unless lease_db.empty?
    ...
    @property_hash = if ensure_state == :present
                       self.class.resource_hash(dhcp6, config_path)
                     else
  ...
  def value_for(key)
    ...
    if [:options, :hooks_libraries].include?(key) && value.nil?
      []
    elsif key == :lease_database && value.nil?
      {}
    else
      value
    end
