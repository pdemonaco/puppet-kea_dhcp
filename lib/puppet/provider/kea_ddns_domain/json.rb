...
Puppet::Type.type(:kea_ddns_domain).provide(:json, parent: PuppetX::KeaDhcp::Provider::Json) do
  ...
  def self.instances
    ...
    forward_domains.each do |domain|
      instances << new(domain_to_hash(domain, 'forward', DEFAULT_CONFIG_PATH))
    end
    ...
    reverse_domains.each do |domain|
      instances << new(domain_to_hash(domain, 'reverse', DEFAULT_CONFIG_PATH))
    end
  ...
  def self.find_domain(ddns, direction, puppet_name, domain_name)
    ...
    found = domains.find do |d|
      d.dig(USER_CONTEXT_KEY, PUPPET_NAME_KEY) == puppet_name
    end
  ...
  def self.domain_to_hash(domain, direction, path)
    puppet_name = domain.dig(USER_CONTEXT_KEY, PUPPET_NAME_KEY) || domain['name']
    {
      ensure: :present,
      name: puppet_name,
  ...
  def flush
    ...
    config[DDNS_KEY] ||= { FORWARD_DDNS_KEY => {}, REVERSE_DDNS_KEY => {} }
    ...
    if @property_flush[:ensure] == :absent
      ...
      domains.reject! do |d|
        d.dig(USER_CONTEXT_KEY, PUPPET_NAME_KEY) == resource[:name] ||
      ...
      domain[USER_CONTEXT_KEY] ||= {}
      domain[USER_CONTEXT_KEY][PUPPET_NAME_KEY] = resource[:name]
    ...
    @property_hash = if ensure_state == :present
                       self.class.domain_to_hash(
                         domains.find { |d| d.dig(USER_CONTEXT_KEY, PUPPET_NAME_KEY) == resource[:name] },
                         direction,
                         config_path,
                       )
                     else
