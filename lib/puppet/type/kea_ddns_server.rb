...
Puppet::Type.newtype(:kea_ddns_server) do
  ...
  INSTANCE_NAME = 'dhcp-ddns'

  newparam(:name, namevar: true) do
    desc 'The unique identifier for the DHCP-DDNS server configuration. Must be dhcp-ddns.'

    validate do |value|
      raise ArgumentError, "Only the '#{INSTANCE_NAME}' DHCP-DDNS server resource is supported" unless value == INSTANCE_NAME
    end
  end
  ...
  newproperty(:port) do
    ...
    validate do |value|
      ...
      raise ArgumentError, 'port must be between 1 and 65_535' unless port.between?(1, 65_535)
    end
